...
protected:
    ...
    EiDeviceConfig *config_mem;

public:
    EiDeviceInfo(EiDeviceConfig *config_mem = nullptr) : config_mem(config_mem) {};
    ...
    virtual bool save_config(void)
    {
        if (config_mem) {
            ...
            config_mem->save_config((uint8_t *)&buf, sizeof(EiConfig));
        }
    ...
    virtual void load_config(void)
    {
        if (config_mem) {
            ...
            memset(&buf, 0, sizeof(EiConfig));
            config_mem->load_config((uint8_t *)&buf, sizeof(EiConfig));
    ...
    /**
     * @brief Must implement!  Stub exists for backward compatibility only
     * 
     * @param address Offset/address into backing store 
     * @param length number of bytes to encode 
     * @return true 
     * @return false 
     */
    virtual bool read_encode_send_sample_buffer(size_t address, size_t length)
    {
        assert(false);
        return 0;
    }
    ...
    virtual void print_wifi_config()
    {
        ei_printf("Present:   0\n");
    }
